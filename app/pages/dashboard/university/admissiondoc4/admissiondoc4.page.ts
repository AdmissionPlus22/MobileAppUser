  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      if (this.uploadForm.get('Doc4').value != "" && this.uploadForm.get('Doc4').value != null) {
        ...
        this.http.post<any>(this.Site_url + '/UniversityDoc4Api', formData).subscribe( dataReturnFromService =>{
   
          // this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          this.router.navigate(['/universityadmissiondoc5/' + this.ApplicationId + '/' + this.DocId ]);

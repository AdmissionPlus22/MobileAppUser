...
@Component({
  selector: 'app-admissiondoc6',
  templateUrl: './admissiondoc6.page.html',
  styleUrls: ['./admissiondoc6.page.scss'],
})
export class Admissiondoc6Page implements OnInit {
  Site_url: string
  UserId: string = "";
  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      this.uploadForm.get('Doc6').setValue(file);
      this.ImagePreview(event)
      if (this.uploadForm.get('Doc6').value != "" && this.uploadForm.get('Doc6').value != null) {
        ...
        formData.append('Doc6', this.uploadForm.get('Doc6').value);
        formData.append('UserId', this.UserId);
        formData.append('ApplicationId', this.ApplicationId);     
        formData.append('DocId', this.DocId);       
  
        this.http.post<any>(this.Site_url + '/UniversityDoc6Api', formData).subscribe( dataReturnFromService =>{
   
          // this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          //this.router.navigate(['/admissiondoc6/' + this.ApplicationId + '/' + this.DocId ]);
          this.router.navigate(['/thankyou']);

          this.showError('Transfer Certificate saved successfully.')
          loading.dismiss()
        }

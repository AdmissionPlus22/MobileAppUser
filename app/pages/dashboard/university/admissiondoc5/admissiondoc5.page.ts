...
@Component({
  selector: 'app-admissiondoc5',
  templateUrl: './admissiondoc5.page.html',
  styleUrls: ['./admissiondoc5.page.scss'],
})
export class Admissiondoc5Page implements OnInit {
  Site_url: string
  UserId: string = "";
  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      this.uploadForm.get('Doc5').setValue(file);
      this.ImagePreview(event)
      if (this.uploadForm.get('Doc5').value != "" && this.uploadForm.get('Doc5').value != null) {
        ...
        formData.append('Doc5', this.uploadForm.get('Doc5').value);
        formData.append('UserId', this.UserId);
        formData.append('ApplicationId', this.ApplicationId);     
        formData.append('DocId', this.DocId);       
  
        this.http.post<any>(this.Site_url + '/UniversityDoc5Api', formData).subscribe( dataReturnFromService =>{
   
          // this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          this.router.navigate(['/universityadmissiondoc6/' + this.ApplicationId + '/' + this.DocId ]);
          //this.router.navigate(['/thankyou']);

          this.showError('Bonafide Certificate saved successfully.')
          loading.dismiss()
        }

...
@Component({
  selector: 'app-admissiondoc3',
  templateUrl: './admissiondoc3.page.html',
  styleUrls: ['./admissiondoc3.page.scss'],
})
export class Admissiondoc3Page implements OnInit {
  Site_url: string
  UserId: string = "";
  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      this.uploadForm.get('Doc3').setValue(file);
      this.ImagePreview(event)
      if (this.uploadForm.get('Doc3').value != "" && this.uploadForm.get('Doc3').value != null) {
        ...
        formData.append('Doc3', this.uploadForm.get('Doc3').value);
        formData.append('UserId', this.UserId);
        formData.append('ApplicationId', this.ApplicationId);     
        formData.append('DocId', this.DocId);       
  
        this.http.post<any>(this.Site_url + '/UniversityDoc3Api', formData).subscribe( dataReturnFromService =>{
   
          // this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          this.router.navigate(['/universityadmissiondoc4/' + this.ApplicationId + '/' + this.DocId ]);

          this.showError('Previous Academic Report saved successfully.')
          loading.dismiss()
        }

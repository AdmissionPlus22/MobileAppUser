  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      if (this.uploadForm.get('Doc3').value != "" && this.uploadForm.get('Doc3').value != null) {
        ...
        this.http.post<any>(this.Site_url + '/CollegeDoc3Api', formData).subscribe( dataReturnFromService =>{
   
          // this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          this.router.navigate(['/collegeadmissiondoc4/' + this.ApplicationId + '/' + this.DocId ]);

  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      if (this.uploadForm.get('Doc2').value != "" && this.uploadForm.get('Doc2').value != null) {
        ...
        this.http.post<any>(this.Site_url + '/CollegeDoc2Api', formData).subscribe( dataReturnFromService =>{
   
          // this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          this.router.navigate(['/collegeadmissiondoc3/' + this.ApplicationId + '/' + this.DocId ]);

...
@Component({
  selector: 'app-admissiondocuments',
  templateUrl: './admissiondocuments.page.html',
  styleUrls: ['./admissiondocuments.page.scss'],
})
export class AdmissiondocumentsPage implements OnInit {
  Site_url: string
  UserId: string = "";
  ...
  async onFileSelect(event) {
    if (event.target.files.length > 0) {
      ...
      if (this.uploadForm.get('Doc1').value != "" && this.uploadForm.get('Doc1').value != null) {
        ...
        this.http.post<any>(this.Site_url + '/Doc1Api', formData).subscribe( dataReturnFromService =>{
   
          this.DocId = dataReturnFromService
          console.log(dataReturnFromService)
          this.router.navigate(['/admissiondoc2/' + this.ApplicationId + '/' + this.DocId ]);
